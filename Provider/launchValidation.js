...
function valid_launch_request(body, req) {
  ...
  if (body.hasOwnProperty("aud")) {
    if (
      (typeof body.aud === "array" &&
        !body.aud.includes(
          req.session.platform_DBinfo.consumerToolClientID &&
            body.hasOwnProperty("azp") &&
            !body.azp !== req.session.platform_DBinfo.consumerToolClientID
        )) ||
  ...
  if (
    ...
    if (
      ...
      body["https://purl.imsglobal.org/spec/lti/claim/target_link_uri"] !==
      req.session.login_request.target_link_uri
    ) {
      errors.push("Target Link URI invalid");
    }
  } else {
  ...
  if (
    body.hasOwnProperty("https://purl.imsglobal.org/spec/lti/claim/context")
  ) {
    if (
      !body["https://purl.imsglobal.org/spec/lti/claim/context"].hasOwnProperty(
        "label"
      ) &&
      !body["https://purl.imsglobal.org/spec/lti/claim/context"].hasOwnProperty(
        "title"
...
function launchTool(req, res, path) {
  ...
  if (req.body.hasOwnProperty("error")) {
    ...
    if (!is_valid_oidc_launch(req)) {
      ...
      axios
        .get(
          req.session.platform_DBinfo.consumerAuthorizationconfig.key +
            "?kid=" +
            basic_decoded.header.kid
        )
        .then((keys) => {
          jwt.verify(
            jwt_string,
