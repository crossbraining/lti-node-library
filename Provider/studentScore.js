...
function prep_send_score(req) {
  if (
    req.session.decoded_launch.hasOwnProperty(
      "https://purl.imsglobal.org/spec/lti-ags/claim/endpoint"
    ) &&
    req.session.decoded_launch[
      "https://purl.imsglobal.org/spec/lti-ags/claim/endpoint"
...
function code_request(req, scope) {
  ...
  return (
    req.session.platform_DBinfo.consumerAuthorizationURL +
    "?" +
    Object.keys(payload)
...
function send_score(req, score, scoreMax) {
  ...
  const base64_user_pass = encode(
    req.session.platform_DBinfo.kid[0].keyID +
      ":" +
      req.session.platform_DBinfo.kid[0].privateKey,
    "base64"
  );

  axios
    .post(req.session.platform_DBinfo.consumerAccessTokenURL, payload, {
      headers: {
