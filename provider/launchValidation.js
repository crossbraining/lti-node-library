 ...
 */
function validLaunchRequest(body, req) {
  const errors = [];
  ...
  if (body['https://purl.imsglobal.org/spec/lti/claim/message_type']) {
    if (
  ...
  if (body['https://purl.imsglobal.org/spec/lti/claim/version']) {
  ...
  if (body.iss) {
    if (body.iss !== req.session.loginRequest.iss) {
      errors.push('Issuer invalid', body.iss, req.session.loginRequest.iss);
    }
  } else {
  ...
  if (body.aud) {
    if (
      (Array.isArray(body.aud)
        && !body.aud.includes(
          req.session.platform.consumerToolClientID
            && body.azp
            && !body.azp !== req.session.platform.consumerToolClientID,
        ))
      || body.aud !== req.session.platform.consumerToolClientID
    ) {
  ...
  if (body['https://purl.imsglobal.org/spec/lti/claim/deployment_id']) {
    if (
  ...
  if (body['https://purl.imsglobal.org/spec/lti/claim/target_link_uri']) {
    if (
      body['https://purl.imsglobal.org/spec/lti/claim/target_link_uri'] === ''
    ) {
      errors.push('Target Link URI invalid');
    } else if (
      body['https://purl.imsglobal.org/spec/lti/claim/target_link_uri']
      !== req.session.loginRequest.target_link_uri
    ) {
      errors.push('Target Link URI invalid');
    }
  } else {
  ...
  if (body['https://purl.imsglobal.org/spec/lti/claim/roles']) {
    if (
  ...
  if (body['https://purl.imsglobal.org/spec/lti/claim/context']) {
    if (
    ...
    if (body['https://purl.imsglobal.org/spec/lti/claim/context'].type) {
      if (
        !body['https://purl.imsglobal.org/spec/lti/claim/context'].type.some(
          (context) => VALID_CONTEXTS.indexOf(context) >= 0,
        )
      ) {
        errors.push(
          'Context invalid: type invalid',
          body['https://purl.imsglobal.org/spec/lti/claim/context'].type,
        );
      }
    } else {
  ...
  if (body.given_name && typeof body.given_name !== 'string') {
  ...
  if (body['https://purl.imsglobal.org/spec/lti-ags/claim/endpoint']) {
    if (
      !body['https://purl.imsglobal.org/spec/lti-ags/claim/endpoint'].scope
      || body['https://purl.imsglobal.org/spec/lti-ags/claim/endpoint'].scope
        .length === 0
    ) {
    ...
    if (
      !body['https://purl.imsglobal.org/spec/lti-ags/claim/endpoint']
        .lineitem
      || typeof body['https://purl.imsglobal.org/spec/lti-ags/claim/endpoint']
        .lineitem !== 'string'
    ) {
      errors.push('Score setup invalid');
    }
...
function isValidOidcLogin(req) {
  ...
  if (req.body.state !== req.session.loginResponse.state) {
    return false;
  }
  return true;
}
...
async function launchTool(req, res, path) {
  ...
  if (req.body.error) {
    ...
    const keys = await axios.get(
      `${req.session.platform.consumerAuthorizationconfig.key}?kid=${basicDecoded.header.kid}`,
    );
    ...
    try {
      ...
      req.session.decodedLaunch = decoded;

      // Validate Launch Request details
      errors = validLaunchRequest(decoded, req);
...
module.exports = { launchTool, validLaunchRequest };
